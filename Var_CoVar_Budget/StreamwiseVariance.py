...
# -*- coding: utf-8 -*-
"""
Created on Tue May 21 11:43:31 2024
...
#ADVECTION
terms_bdg['adv_h'] = - data_tavg['u']*get_dphidx(terms_ptb['u2_t'], wn_x) - data_tavg['v']*get_dphidy(terms_ptb['u2_t'], wn_y)
terms_bdg['adv_v'] = - data_tavg['w']*wnode2uvpnode(get_dphidz(terms_ptb['u2_t'], dz))

terms_bdg['adv_h_SGS'] = - data_tavg['u']*get_dphidx(data_tavg['txx'], wn_x) - data_tavg['v']*get_dphidy(data_tavg['txx'], wn_y)
terms_bdg['adv_v_SGS'] = - data_tavg['w']*wnode2uvpnode(get_dphidz(data_tavg['txx'], dz))

terms_bdg['adv'] = terms_bdg['adv_h'] + terms_bdg['adv_v'] + terms_bdg['adv_h_SGS'] + terms_bdg['adv_v_SGS']

#PRODUCTION
terms_bdg['prod_h'] = - 2*(terms_ptb['u2_t']*data_tavg['dudx'] + terms_ptb['uv_t']*data_tavg['dudy'])
terms_bdg['prod_v'] = - 2*(terms_ptb['uw_t']*data_tavg['dudz'])

terms_bdg['prod_h_SGS'] = - 2*(data_tavg['txx']*data_tavg['dudx'] + data_tavg['txy']*data_tavg['dudy'])
terms_bdg['prod_v_SGS'] = - 2*(data_tavg['txz']*data_tavg['dudz'])
...
terms_bdg['uturb_h'] = - get_dphidx(terms_ptb['u3_t'], wn_x) - get_dphidy(terms_ptb['u2v_t'], wn_y) - get_dphidx(terms_ptb['utxx_t'], wn_x)\
    - get_dphidy(terms_ptb['utxy_t'], wn_y)
...
terms_bdg['uturb_v'] = - wnode2uvpnode(get_dphidz(terms_ptb['wu2_t'], dz)) - wnode2uvpnode(get_dphidz(terms_ptb['utxz_t'], dz))

terms_bdg['ttrans'] = terms_bdg['uturb_h'] + terms_bdg['uturb_v']

#PRESSURE TRANSPORT
# p_clean = data_tavg['p'] - (1/3)*(data_tavg['txx'] + data_tavg['tyy'] + data_tavg['tzz'])
# terms_ptb['pu_t'] = (data_tavg['pu'] - (1/3)*(data_tavg['utxx'] + data_tavg['utyy'] + data_tavg['utzz'])) - p_clean*data_tavg['u']
terms_ptb['pu_t'] = (data_tavg['pu']) - data_tavg['p']*data_tavg['u']
terms_bdg['pturb_t'] = - 2*(get_dphidx(terms_ptb['pu_t'], wn_x))
terms_ptb['pdudx_t'] = data_tavg['pdudx'] - data_tavg['p']*data_tavg['dudx']
terms_bdg['pturb_s'] = 2*terms_ptb['pdudx_t'] #- 1.5

terms_bdg['ptrans'] = terms_bdg['pturb_t'] + terms_bdg['pturb_s']

#DISSIPATION
# terms_ptb['dudx2_t'] = data_tavg['dudx2'] - data_tavg['dudx']*data_tavg['dudx']
# terms_ptb['dudy2_t'] = data_tavg['dudy2'] - data_tavg['dudy']*data_tavg['dudy']
# terms_ptb['dudz2_t'] = data_tavg['dudz2'] - data_tavg['dudz']*data_tavg['dudz']
# terms_bdg['dissip'] = - 2*(terms_ptb['dudx2_t'] + terms_ptb['dudy2_t'] + terms_ptb['dudz2_t'])
...
data_tavg['dissip'] = data_tavg['dxx'] + data_tavg['dxy'] + data_tavg['dxz']

terms_bdg['dissip'] = 2*(data_tavg['dissip']\
  - data_tavg['txx']*terms_drv['S11'] - data_tavg['txy']*terms_drv['S12'] - data_tavg['txz']*terms_drv['S13'])
    
#CANOPY
terms_bdg['canopy'] = 2*(data_tavg['ufdx']-data_tavg['u']*data_tavg['fdx'])
...
# np.save(pathOUT + 'UU_terms.npy', terms_bdg) 
...
plt.xlim(-50,50)
plt.title(r'Streamwise Variance')
plt.xlabel(r'$\left \langle duu/dt \right \rangle$')
# plt.xlabel(r'$\left \langle de/dt \right \rangle$')
plt.ylabel(r'$z/h_c$')
plt.legend(loc='upper right')

# plt.savefig(pathOUT +'Figures/UU_budget_prof_flat.png',dpi=300,facecolor='white', edgecolor='white')
