...
# -*- coding: utf-8 -*-
"""
Created on Tue May 27 01:53:10 2025
...
for i in range(0,Nfiles):
    ...
    sigma_a[:,i] = np.mean(uu_w - u_w*u_w,axis=(0,1))
    ...
    # sigma_a[:,i] = np.mean((vv - v*v),axis=(0,1))
    ...
    # sigma_a[:,i] = np.mean((ww - w*w),axis=(0,1))
    ...
    uu_w = uvpnode2wnode(uu)
    sigma_na[:,i] = np.mean(uu_w - u_w*u_w,axis=(0,1))
    ...
    # # vv_w = uvpnode2wnode(vv)
    # sigma_na[:,i] = np.mean((vv - v*v),axis=(0,1))
    ...
    # sigma_na[:,i] = np.mean((ww - w*w),axis=(0,1))
...
p1 = axs[0].pcolormesh(mdates.date2num(time_vector),np.arange(0,nz)*dz*zi,sigma_a*(uscale**2)/(ug**2)*1000,vmin=0,vmax=4,cmap='hot',shading='auto')
p2 = axs[1].pcolormesh(mdates.date2num(time_vector),np.arange(0,nz)*dz*zi,sigma_na*(uscale**2)/(ug**2)*1000,vmin=0,vmax=4,cmap='hot',shading='auto')
...
cbar.set_label(label=r'$\sigma/u_G^2$',fontsize=15)
cbar = plt.colorbar(p2)
cbar.set_label(label=r'$\sigma/u_G^2$',fontsize=15)

# plt.savefig(path_fig+'sigmaW_ts_prof.png',dpi=300)
plt.show()
...
axs[0].set_ylabel(r"$\sigma/u_G^2$",fontsize=15)
axs[1].set_ylabel(r"$\sigma/u_G^2$",fontsize=15)
axs[1].set_xlabel('Time',fontsize=15)
...
# plt.savefig(path_fig+'sigmaW_ts_heights.png',dpi=300)
plt.show()
