...
def ReynoldsStress_2D(Nx,Nz,avgU,avgV,avgW,avgU2,avgV2,avgW2,avgUV,avgUW,avgVW):
    "Calling the ReynoldsStress() function"
    ...
    # Projection of u & v on w-nodes: -------------------------
    avgU_w = np.zeros((Nx,Nz),'d',order='F')
    avgV_w = np.zeros((Nx,Nz),'d',order='F')
    ...
    # Projection of w on uvpT-nodes: -------------------------
    avgW_uvpT = np.zeros((Nx,Nz),'d',order='F')
    avgW2_uvpT = np.zeros((Nx,Nz),'d',order='F')
...
def ReynoldsStressUVP(Nx,Ny,Nz,avgU,avgV,avgW,avgU2,avgV2,avgW2,avgUV,avgUW,avgVW):
    "Calling the ReynoldsStress() function"
    ...
    Rstress[:,:,:,0] = avgU2 - avgU*avgU #Rxx at uvpT node.
    Rstress[:,:,:,1] = avgV2 - avgV*avgV #Ryy at uvpT node.
    Rstress[:,:,:,2] = avgW2 - avgW*avgW #Rzz at uvpT node.
    Rstress[:,:,:,3] = avgUV - avgU*avgV #Rxy; Changing the corresponding sign!  at uvpT node.
    Rstress[:,:,:,4] = avgUW - avgU*avgW #Rxz; Changing the corresponding sign!  at w node.
    Rstress[:,:,:,5] = avgVW - avgV*avgW #Ryz; Changing the corresponding sign!  at w node.
...
def ReynoldsStressUVP_SGS(Nx,Ny,Nz,avgU,avgV,avgW,avgU2,avgV2,avgW2,avgUV,avgUW,avgVW,avgtxx,avgtyy,avgtzz,avgtxy,avgtxz,avgtyz):
    "Calling the ReynoldsStress() function"
    ...
    # Computing the different terms
    Rstress[:,:,:,0] = avgU2 - avgU*avgU + avgtxx #Rxx at uvpT node.
    Rstress[:,:,:,1] = avgV2 - avgV*avgV + avgtyy #Ryy at uvpT node.
    Rstress[:,:,:,2] = avgW2 - avgW*avgW + avgtzz #Rzz at uvpT node.
    Rstress[:,:,:,3] = avgUV - avgU*avgV + avgtxy #Rxy; Changing the corresponding sign!  at uvpT node.
    Rstress[:,:,:,4] = avgUW - avgU*avgW + avgtxz #Rxz; Changing the corresponding sign!  at w node.
    Rstress[:,:,:,5] = avgVW - avgV*avgW + avgtyz #Ryz; Changing the corresponding sign!  at w node.
...
def ReynoldsStressUVP_2D(Nx,Nz,avgU,avgV,avgW,avgU2,avgV2,avgW2,avgUV,avgUW,avgVW):
    "Calling the ReynoldsStress() function"
...
def DispFluct(Nx,Ny,Nz,avgVar):
    ...
    dispFluct = avgVar - np.nanmean(np.nanmean(avgVar,axis = 0),axis=0)
