...
# -*- coding: utf-8 -*-
"""
Created on Thu May 29 08:26:24 2025
...
for i in range(0,Nfiles):
    ...
    ww = xr.open_dataarray(path_a+'Data_Momentum_'+str(StartTime + AvgIter*i)+'.nc').data[:,:,0:50,6]
    tzz = xr.open_dataarray(path_a+'Data_Momentum_'+str(StartTime + AvgIter*i)+'.nc').data[:,:,0:50,18]
    tzz_w = uvpnode2wnode(tzz)
    sigma_a[str(StartTime + AvgIter*i)] = (ww - w*w - tzz_w)
    ...
    ww = xr.open_dataarray(path_na+'Data_Momentum_'+str(StartTime + AvgIter*i)+'.nc').data[:,:,0:50,6]
    tzz = xr.open_dataarray(path_na+'Data_Momentum_'+str(StartTime + AvgIter*i)+'.nc').data[:,:,0:50,18]
    tzz_w = uvpnode2wnode(tzz)
    sigma_na[str(StartTime + AvgIter*i)] = (ww - w*w - tzz_w)
...
for i in range(len(y_b_vec)):
    axs1.semilogx(-zeta_vec_u,phi_w_fit_u[:,i],c=cmap(y_b_vec[i]))
    axs2.semilogx(zeta_vec_s,phi_w_fit_s[:,i],c=cmap(y_b_vec[i]))
    axs3.semilogx(-zeta_vec_u,phi_w_fit_u[:,i],c=cmap(y_b_vec[i]))
    axs4.semilogx(zeta_vec_s,phi_w_fit_s[:,i],c=cmap(y_b_vec[i]))
...
axs1.set_ylabel(r'$\phi_w$',fontsize=15)
axs4.set_xlabel(r'$\zeta$',fontsize=15)
axs3.set_ylabel(r'$\phi_w$',fontsize=15)
...
# plt.savefig(path_fig+'sigmaW_vs_zL.png',dpi=300)
plt.show()
...
axs.scatter_density(abs(zeta_tot_na[(zeta_tot_na>0)]),phi_na[(zeta_tot_na>0)],cmap='hot_r')
axs.set_ylim(0,10)
axs.set_xscale('log')
for i in range(len(y_b_vec)):
    axs.semilogx(zeta_vec_s,phi_w_fit_s[:,i],c=cmap(y_b_vec[i]))
...
plt.scatter(x_sample,y_sample,c='k')
...
# pdf, xedges, yedges = np.histogram2d(x_sample,y_sample,bins=100,density=True)
...
h,xedge,yedge,img=plt.hist2d(x_sample,y_sample,bins=[log_bin_x,bin_y],cmap='viridis',density=False)
h[h==0] = np.nan
...
for i in range(len(y_b_vec)):
    plt.semilogx(-zeta_vec_u,phi_w_fit_u[:,i],c=cmap(y_b_vec[i]))
