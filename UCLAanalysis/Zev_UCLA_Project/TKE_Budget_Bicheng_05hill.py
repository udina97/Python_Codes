...
path = '/uufs/chpc.utah.edu/common/home/calaf-group3/UCLA_data/data/amazon_canopy_hill05h_3D/'
...
#Amazon Hill Canopy
Nx = 320; Ny = 160; Nz = 264
Lx = 2000; Ly = 1000; Lz = 528
Zi = 528
...
z_tpg = np.zeros((dm.nx))
x_tpg = np.arange(0,dm.nx)*dm.dx

for i in range(0,dm.nx):
    z_tpg[i] = (25/2)*np.cos((np.pi/(2*250))*x_tpg[i]+np.pi) + (25/2)
    
Z_tg = z_tpg/dm.dz

zeds = np.zeros((dm.nx,dm.nz))

for i in range(0,dm.nx):
    zeds[i,:] = np.arange(0,dm.nz)*dm.dz
    
dist = copy.deepcopy(zeds)

for i in range(0,dm.nx):
    for k in range(0,dm.nz):
        dist[i,k] = dist[i,k] - z_tpg[i]
        
#%% Loading the 2D .out dm.data

path_in = '/uufs/chpc.utah.edu/common/home/calaf-group3/UCLA_data/amazon_canopy_hill05h/output/'
path_out = '/uufs/chpc.utah.edu/common/home/calaf-group2/Ben_research/UCLAdata/amazon_canopy_hill05/'
...
path_fig = '/uufs/chpc.utah.edu/common/home/u1450851/Pictures/UCLA_tke/hill05/'
...
# plt.plot(np.nanmean(terms_bdg['prod'],axis=(0))/np.nanmean(-terms_bdg['totdis'],axis=(0)),z_ax,c='blue',marker='D',markevery=7,label='Prod')
...
axs.plot(np.arange(0,Nx)*Dx,z_tpg/canopyH,color='k')
axs.plot(np.arange(0,Nx)*Dx,(z_tpg+canopyH)/canopyH,color='k')
...
for i in range(0,len(axs)):
    for j in range(0,len(axs[0])):
        if i == 0:
            ...
            axs[i,j].plot(np.arange(0,Nx)*Dx,z_tpg/canopyH,color='k')
            axs[i,j].plot(np.arange(0,Nx)*Dx,(z_tpg+canopyH)/canopyH,color='k')
...
axs[0].plot((x_ax),(z_tpg + h_canopy)/dm.zi,'--k')
axs[1].plot((x_ax),(z_tpg + h_canopy)/dm.zi,'--k')
axs[2].plot((x_ax),(z_tpg + h_canopy)/dm.zi,'--k')
...
plt.xlim(-2,2)
if condition=='true':
    plt.title(r'Hill - Planar')
else:
    plt.title(r'Hill05 - Terrain')
...
# plt.plot(np.nanmean(terms_bdg['ttrans'],axis=(0)),z_ax,c='black',marker='s',markevery=5,label='<T>')
plt.hlines(h_canopy/h_canopy,-60,80,linestyle='--',colors='k')
plt.hlines(2*(h_canopy/h_canopy),-60,80,linestyle='--',colors='k')
plt.ylim(0,10)
plt.xlim(-40,65)
if condition=='true':
    plt.title(r'Hill05 - Planar')
else:
    plt.title(r'Hill05 - Terrain')
...
axs[0].plot((x_ax/dm.zi),(z_tpg + h_canopy)/dm.zi,'--k')
axs[1].plot((x_ax/dm.zi),(z_tpg + h_canopy)/dm.zi,'--k')
...
axs.plot((x_ax/dm.zi),(z_tpg + h_canopy)/dm.zi,'--k')
...
axs.streamplot(x_ax, z_ax, DispFluct_withTopo(data_tavg['u'],dist,dm).T, DispFluct_withTopo(data_tavg['w'],dist,dm).T,
               density = 1,color=[0.7,0.7,0.7])
axs.plot((x_ax),(z_tpg + h_canopy )/dm.zi,'--k')
axs.set_title('Vorticity - y & Dispersive Streamlines')
