...
# -*- coding: utf-8 -*-
"""
Created on Fri May 23 03:04:32 2025
...
path_a = '/scratch/general/nfs1/u1450851/LES_Sims/'+sim_a+'/data/Anisotropy/'
sim_na = 'diurnal_c_noaniso_L3D'
path_na = '/scratch/general/nfs1/u1450851/LES_Sims/'+sim_na+'/data/Anisotropy/'
StartTime = 1296000
AvgIter = 24000
...
for i in range(Nfiles):
    yB_a[str(StartTime + AvgIter*i)] = xr.open_dataarray(path_a+'Data_Anisotropy_'+str(StartTime + AvgIter*i)+'.nc').data[:,:,:,1]
    yB_na[str(StartTime + AvgIter*i)] = xr.open_dataarray(path_na+'Data_Anisotropy_'+str(StartTime + AvgIter*i)+'.nc').data[:,:,:,1]
    
yB_2D_a = np.zeros((nz,Nfiles),order='F')
yB_2D_na = np.zeros((nz,Nfiles),order='F')

for i in range(Nfiles):
    yB_2D_a[:,i] = np.mean(yB_a[str(StartTime + AvgIter*i)],axis=(0,1))
    yB_2D_na[:,i] = np.mean(yB_na[str(StartTime + AvgIter*i)],axis=(0,1))
...
p1 = axs[0].pcolormesh(mdates.date2num(time_vector),np.arange(0,nz)*dz*zi,yB_2D_a,cmap=ColorAnisotropy(),vmin=0,vmax=np.sqrt(3)/2,shading='auto')
p2 = axs[1].pcolormesh(mdates.date2num(time_vector),np.arange(0,nz)*dz*zi,yB_2D_na,cmap=ColorAnisotropy(),vmin=0,vmax=np.sqrt(3)/2,shading='auto')
...
cbar.set_label(label=r'$y_B$',fontsize=15)
cbar = plt.colorbar(p2)
cbar.set_label(label=r'$y_B$',fontsize=15)

# plt.savefig(path_fig+'yB_ts_prof.png',dpi=300)
plt.show()
...
for i in range(len(height)):
    p1 = axs[0].plot(mdates.date2num(time_vector),yB_2D_a[height[i],:],label=f'{height[i]*dz*zi:.2f}')
    p2 = axs[1].plot(mdates.date2num(time_vector),yB_2D_na[height[i],:],label=f'{height[i]*dz*zi:.2f}')
    
axs[0].set_ylabel(r"$y_B$",fontsize=15)
axs[1].set_ylabel(r"$y_B$",fontsize=15)
axs[1].set_xlabel('Time',fontsize=15)
...
# plt.savefig(path_fig+'yB_ts_heights.png',dpi=300)
plt.show()
