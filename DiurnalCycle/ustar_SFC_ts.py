...
# -*- coding: utf-8 -*-
"""
Created on Fri May 16 10:01:08 2025
...
path_a = '/scratch/general/nfs1/u1450851/LES_Sims/'+sim_a+'/data/Momentum2D/'
sim_na = 'diurnal_c_noaniso_L3D_fix'
path_na = '/scratch/general/nfs1/u1450851/LES_Sims/'+sim_na+'/data/Momentum2D/'
StartTime = 1296000
AvgIter = 24000
...
for i in range(0,Nfiles):
    # dataM2D_a = xr.open_dataarray(path_a+'Data_Momentum_2D_'+str(StartTime + AvgIter*i)+'.nc').data[:,:,0]
    ustar_a[str(StartTime + AvgIter*i)] = xr.open_dataarray(path_a+'Data_Momentum_2D_'+str(StartTime + AvgIter*i)+'.nc').data[:,:,0]
    # dataM2D_na = xr.open_dataarray(path_na+'Data_Momentum_2D_'+str(StartTime + AvgIter*i)+'.nc').data[:,:,0]
    ustar_na[str(StartTime + AvgIter*i)] = xr.open_dataarray(path_na+'Data_Momentum_2D_'+str(StartTime + AvgIter*i)+'.nc').data[:,:,0]
    # del dataM2D_a,dataM2D_na
...
uS_K = pd.read_csv(path_csv+'Kumar_ustar.csv',header=None,index_col=False)
uS_V = pd.read_csv(path_csv+'Varun_ustar.csv',header=None,index_col=False)
...
#     axs.plot(event_time,np.mean(ustar_a[str(StartTime + AvgIter*i)])*uscale,'o',c='k',label='Aniso')
#     axs.plot(event_time,np.mean(ustar_na[str(StartTime + AvgIter*i)])*uscale,'o',c='r',label='NoAniso')
    
for i in range(0,Nfiles):
    event_time = start_time + timedelta(hours=(StartTime + AvgIter*i)*dt/3600)
    axs.plot((StartTime + AvgIter*i)*0.075/3600,np.mean(ustar_a[str(StartTime + AvgIter*i)])*uscale,'o',c='k',label='Aniso')
    axs.plot((StartTime + AvgIter*i)*0.075/3600,np.mean(ustar_na[str(StartTime + AvgIter*i)])*uscale,'o',c='r',label='NoAniso')
    
axs.plot(uS_K[0]+7,uS_K[1],c='k',label='Kumar')
axs.plot(uS_V[0]+7,uS_V[1],c='r',label='Varun')
    
axs.set_ylabel(r'$u_*$ $[m s^{-1}]$',fontsize=15)
...
# plt.savefig(path_fig+'ustar_ts_sfc.png',dpi=300)
plt.show()
