...
path = '/scratch/general/nfs1/u1450851/UCLAdata/data/amazon_canopy_flat_3D/'
...
# meandUdz = np.zeros((Nx,Ny,Nz),'d',order='F')
# meandUdz_1D = np.zeros((Nz),'d',order='F')
# phi_mom = np.zeros((Nx,Ny,Nz),'d',order='F')
# u = data.data[:,:,:,6]
# v = data.data[:,:,:,7]
# tmp_ustar = (((-Rstress.data[:,:,:,4]+data.data[:,:,:,4])**2 + (-Rstress.data[:,:,:,5] + data.data[:,:,:,5])**2)**(1/4))

# # ustar = np.amax((((-Rstress.data[:,:,:,4]+data.data[:,:,:,4])**2 + (-Rstress.data[:,:,:,5] + data.data[:,:,:,5])**2)**(1/4)),axis=2) #ustar is a 2D matrix of the max values of the shear stress.

# ustar = tmp_ustar[:,:,height+1]

# for k in range(0,Nz): 
#     meandUdz[:,:,k] = ((u[:,:,k]*dUdz[:,:,k]) + (v[:,:,k]*dVdz[:,:,k]))/(np.sqrt(u[:,:,k]**2 + v[:,:,k]**2))
#     # meandUdz_1D[k] = ((np.mean(u[:,:,k],axis=(0,1))*np.mean(dUdz[:,:,k],axis=(0,1))) + (np.mean(v[:,:,k],axis=(0,1))*np.mean(dVdz[:,:,k],axis=(0,1))))/(np.sqrt(np.mean(u[:,:,k],axis=(0,1))**2 + np.mean(v[:,:,k],axis=(0,1))**2))
#     phi_mom[:,:,k] = kappa*z[k]*meandUdz[:,:,k]*0.4

z_grid = np.ones(Nx,'d',order='F') #vector needed to take ustar 1 grid point above the top of the canopy in ppppphi_m func

[phi_m_3D,U,ustar,z_ustar,tmp_ustar] = phi_m(Nx,Ny,Nz,height,z_d,z_grid,data.data[:,:,:,6],data.data[:,:,:,7],dUdz,dVdz,
...
phi_m_xy = np.mean(phi_m_3D,axis=(0,1))

#%%
tmp_ustar_1D = np.mean(tmp_ustar,axis=(0,1))

figure = plt.figure()
plt.plot(tmp_ustar_1D,np.arange(0,Nz))
...
path_fig = '/uufs/chpc.utah.edu/common/home/u1450851/Pictures/Anisotropy_Analysis_UCLA/Flat/marc_poster/'
...
# plt.savefig(path_fig+'yB_flat.png',dpi=300,facecolor='white',edgecolor='white')
...
for m in range(0,3):
    for n in range(0,3):
        ...
        for i in range(0,Nz_SLayer-1):
            sigmaU_median[i] = np.median(tmp_phi_u[(tmp_z3D >= z_d[i]) & (tmp_z3D < z_d[i+1])])
...
for cluster in range(1,Nclusters):
    ...
    #First we downselect data based on the yB clustering developed earlier.
    # tmp_phi_u = phi_M_1D[(AnisType_1D == cluster)]
    tmp_sigma_u = sigma_u_1D[(AnisType_1D == cluster)]
    tmp_yB = yB_1D[(AnisType_1D == cluster)]
    tmp_z3D = z3D_1D[(AnisType_1D == cluster)]

    for k in range(0,Nz_SLayer-1):            
        
        # phiU_median[cluster,k] = np.median(tmp_phi_u[(tmp_z3D >= z_m[k]) & (tmp_z3D < z_m[k+1])]) 
        sigmaU_median[cluster,k] = np.median(tmp_sigma_u[(tmp_z3D >= z_m[k]) & (tmp_z3D < z_m[k+1])]) 
        yB_median[cluster,k] = np.median(tmp_yB[(tmp_z3D >= z_m[k]) & (tmp_z3D < z_m[k+1])]) 
...
#%% Plotting a shaded density map of sigma or phi with ontop the mean/median profiles to explain the spread
from scipy.stats import kde
...
# axs.plot(np.mean(phi_m_3D[:,:,0:Nz_SLayer],axis=(0,1)),z_d[0:Nz_SLayer]/canopyH,color='red') #x-y averaged profile
...
for cluster in range(1,Nclusters):
    ...
    #First we downselect data based on the yB clustering developed earlier.
    tmp_sigma_u = phi_M_1D[(AnisType_1D == cluster)]
    tmp_yB = yB_1D[(AnisType_1D == cluster)]
    tmp_z3D = z3D_1D[(AnisType_1D == cluster)]
...
# plt.savefig(path_fig+'phiM_median_shading.png',dpi=300,facecolor='white',edgecolor='white')
...
anis_clust_pnt = AnisType_3D[int(rdm_pnt[0]),int(rdm_pnt[1]),:]
var_profile = phi_m_3D[int(rdm_pnt[0]),int(rdm_pnt[1]),:]

Nclusters = 10

var_median = np.zeros((Nclusters,Nz_SLayer),dtype='float')

for cluster in range(1,Nclusters):

    #First we downselect data based on the yB clustering developed earlier.
    tmp_var = phi_M_1D[(AnisType_1D == cluster)]
    tmp_z3D = z3D_1D[(AnisType_1D == cluster)]
...
fig, axs = plt.subplots(1,1,figsize=(8,6))
plt.ion()
axs.plot(profile,z_d/canopyH,color='black',label='Built')
axs.plot(var_profile,z_d/canopyH,color='red',label='Original')
axs.axhline(y = (canopyH-30.2/zi)/canopyH, xmin=-10, xmax=8,color='gray',linestyle='--')
axs.axhline(y = (2*canopyH-30.2/zi)/canopyH, xmin=-10, xmax=8,color='gray',linestyle='-.')
# axs.axvline(x =1, ymin = 0, ymax=13,color='gray',linestyle='--')
