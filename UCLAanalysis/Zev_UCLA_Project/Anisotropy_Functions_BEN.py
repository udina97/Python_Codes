...
def Anisotropy_Prod(Nx,Ny,Nz,R11,R22,R33,P11,P12,P13,P21,P22,P23,P31,P32,P33):
    "Calling the Anisotorpy() function"
    ...
    for k in range(0,Nz):
        ...
        for i in range(0,Nx):
            for j in range(0,Ny):
                ...
                # ... Reynolds Matrix
                P = np.matrix([[P11[i,j,k], P12[i,j,k], P13[i,j,k]],
                               [P21[i,j,k], P22[i,j,k], P23[i,j,k]],
                               [P31[i,j,k], P32[i,j,k], P33[i,j,k]]])
...
def phi_m(Nx,Ny,Nz_SLayer,height,z3D,z_grid,u,v,avgdUdz,avgdVdz,Rxz,SGSxz,Ryz,SGSyz):
    ...
    for i in range(0,Nx):
        for j in range(0,Ny):
            k = np.copy(z_grid[i,j]+height)
            ustar[i,j] = tmp_ustar[i,j,int(k)]
            

    
    # ustar = np.amax((((-Rxz+SGSxz)**2 + (-Ryz + SGSyz)**2)**(1/4)),axis=2) #ustar is a 2D matrix of the max values of the shear stress.
    # ustar = tmp_ustar[:,:,21]
    
    
    #Loop to find the height where the ustar Max is found. This is the ustar used to normalize the velocity gradient.
    #z_d_short = z_d[0:Nz_SLayer]
    z_d_ustar = z_grid + height

    #for i in range(0,Nx):
    #    for j in range(0,Ny):
    #        a = tmp_ustar[i,j,:]
    #        b = ustar[i,j]
    #        z_d_ustar[i,j] = z_d_short[(a == b)]
    ...
    for k in range(0,Nz_SLayer): 
        ...
        phi_m_3D[:,:,k] =  (kappa*z3D[:,:,k]/ustar[:,:])*meandUdz[:,:,k]
        # phi_m_3D[:,:,k] =  (kappa*z_d[k])*meandUdz[:,:,k]




    return(phi_m_3D,U,ustar,z_d_ustar,tmp_ustar)
...
def Anisotropy_Clustering_Prod(Nx,Ny,Nz_SLayer,Rstress,Prod):
    ...
    [xB,yB] = Anisotropy_Prod(Nx,Ny,Nz_SLayer,Rstress.data[:,:,:,0],Rstress.data[:,:,:,1],Rstress.data[:,:,:,2],
                         Prod.data[:,:,:,0],Prod.data[:,:,:,1],Prod.data[:,:,:,2],Prod.data[:,:,:,3],Prod.data[:,:,:,4],
                         Prod.data[:,:,:,5],Prod.data[:,:,:,6],Prod.data[:,:,:,7],Prod.data[:,:,:,8])
...
def PhiM_Data_Neutral(Nx,Ny,Nz,Nz_SLayer,dz,zi,data):
    ...
    os.chdir("/home/benjamin/PhD/LES/PythonCodes/")
    ...
    #From the 3D LES files:
    ustar = ((-Rstress.data[:,:,0:Nz_SLayer,4]+data.data[:,:,0:Nz_SLayer,2])**2 + (-Rstress.data[:,:,0:Nz_SLayer,5] + data.data[:,:,0:Nz_SLayer,4])**2)**(1/4)
