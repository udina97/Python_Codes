...
# -*- coding: utf-8 -*-
"""
Created on Mon May 19 13:59:18 2025
...
Rxz_a = np.zeros((nz,Nfiles),order='F')
Ryz_a = np.zeros((nz,Nfiles),order='F')
Rxz_na = np.zeros((nz,Nfiles),order='F')
Ryz_na = np.zeros((nz,Nfiles),order='F')
...
for i in range(0,Nfiles):
    ...
    Rxz_a[:,i] = np.mean(uw - u_w*w - txz,axis=(0,1))
    Ryz_a[:,i] = np.mean(vw - v_w*w - tyz,axis=(0,1))
    ...
    Rxz_na[:,i] = np.mean(uw - u_w*w - txz,axis=(0,1))
    Ryz_na[:,i] = np.mean(vw - v_w*w - tyz,axis=(0,1))
...
p1 = axs[0].pcolormesh(mdates.date2num(time_vector),np.arange(0,nz)*dz*zi,ustar_a*(uscale),cmap='hot',vmin=0,vmax=0.5,shading='auto')
p2 = axs[1].pcolormesh(mdates.date2num(time_vector),np.arange(0,nz)*dz*zi,ustar_na*(uscale),cmap='hot',vmin=0,vmax=0.5,shading='auto')
...
cbar.set_label(label=r'$u_*$ [m/s]',fontsize=15)
cbar = plt.colorbar(p2)
cbar.set_label(label=r'$u_*$ [m/s]',fontsize=15)

# plt.savefig(path_fig+'MF_ts_prof.png',dpi=300)
plt.show()
...
for i in range(len(height)):
    p1 = axs[0].plot(mdates.date2num(time_vector),ustar_a[height[i],:]*uscale,label=f'{height[i]*dz*zi:.2f}')
    p2 = axs[1].plot(mdates.date2num(time_vector),ustar_na[height[i],:]*uscale,label=f'{height[i]*dz*zi:.2f}')
    
axs[0].set_ylabel(r"$u_*$ [m/s]",fontsize=15)
axs[1].set_ylabel(r"$u_*$ [m/s]",fontsize=15)
axs[1].set_xlabel('Time',fontsize=15)
...
# plt.savefig(path_fig+'MF_ts_heights.png',dpi=300)
plt.show()
