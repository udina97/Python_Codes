...
#%% Import the time averaged varibles from the netcdf files
#3D wind field
...
#Pressure terms
PU1 = dataTKE.data[:,:,:,24]; PV1 = dataTKE.data[:,:,:,25]; PW1 = dataTKE.data[:,:,:,26] #pressure terms cleaned from rotational term 
# PU2 = dataTKE.data[:,:,:,40]; PV2 = dataTKE.data[:,:,:,41]; PW2 = dataTKE.data[:,:,:,42] #pressure terms cleaned from rotational term and trace of SGS tensor
PU3 = dataTKE.data[:,:,:,43]; PV3 = dataTKE.data[:,:,:,44]; PW3 = dataTKE.data[:,:,:,45] #pressure terms using LES pressure (not correct, pressure transport too big)
...
#Dissipation terms time averaged "on the fly" in the LES
...
e = 0.5*(uu_var + vv_var + ProjectOn_uvp(Nx, Ny, Nz, ww_var)) #tke
e_sgs = 0.5*(txx + tyy + tzz) #tke_sgs

#%% Tets cell
# var1 = PU1
# var2 = PU3 - 0.5*(UUU+UVV+WUU)
# var3 = pw3_var - 0.5*(WUU+WVV+WWW) - (1/3)*(Wtxx+Wtyy+Wtzz) \
#        + 0.5*(W*ProjectOn_uvp(Nx, Ny, Nz, UU)+W*ProjectOn_uvp(Nx, Ny, Nz, VV)+W*WW) \
#        + (1/3)*(W*ProjectOn_uvp(Nx, Ny, Nz, txx)+W*ProjectOn_uvp(Nx, Ny, Nz, tyy)+W*ProjectOn_uvp(Nx, Ny, Nz, tzz))

# figure,ax = plt.subplots()
# plt.plot(mean_xy(pw1_var),z,color='black',label='pw1_var')
# # plt.plot(mean_xy(pv3_var),z,color='green',label='pv3')
# # plt.plot(mean_xy(0.5*(UU + VV + WW)),z,color='red',label='rot',marker='.')
# plt.plot(mean_xy(var3),z,color='green',label='var3')
# # plt.plot(mean_xy((1/3)*(txx + tyy + tzz)),z,color='red',label='trace_sgs',marker='.')

# # im1 = plt.pcolormesh(x,z,np.transpose(pu3_var[:,30,:]),shading='gouraud',cmap='coolwarm')
# # cbar1 = figure.colorbar(im1, ax=ax, shrink=0.8,location='top',orientation='horizontal',pad=0.1)
# plt.legend()
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
#%%Some functions to Project and Derive 1D varibles in vertical direction
...
dedz = ProjectOn_uvp(Nx, Ny, Nz, ddz3_uv(Nx, Ny, Nz, dz, e))

Adv_res = - (U*dedx + V*dedy + ProjectOn_uvp(Nx, Ny, Nz, W)*dedz)

#SGS
dedx_sgs = Fddx(Nx, Ny, Nz, e_sgs, 1, 0, Lx, Nx)
dedy_sgs = Fddy(Nx, Ny, Nz, e_sgs, 1, 0, Ly, Ny)
dedz_sgs = ProjectOn_uvp(Nx, Ny, Nz, ddz3_uv(Nx, Ny, Nz, dz, e_sgs))

Adv_sgs = -(U*dedx_sgs + V*dedy_sgs + ProjectOn_uvp(Nx, Ny, Nz, W)*dedz_sgs)

#tot
Adv_tot = Adv_res + Adv_sgs

#Advection Finnigan
Adv_F = - mean_xy(ProjectOn_uvp(Nx, Ny, Nz, W))*ProjectOn_uvp_1D(Nz, (ddz_1D_uvp(Nz, dz, mean_xy(e))))

#Advection Finnigan SGS
Adv_sgs_F = - mean_xy(ProjectOn_uvp(Nx, Ny, Nz, W))*ProjectOn_uvp_1D(Nz, (ddz_1D_uvp(Nz, dz, mean_xy(e_sgs))))
...
T0 = mean_xy(T)  #check the LES code for how fabien computed the Beta parameter, planar average for each level
                         # or used a filter around the grid point

WTuvp = ProjectOn_uvp(Nx, Ny, Nz, WT)
Wuvp = ProjectOn_uvp(Nx, Ny, Nz, W)
WTsgsuvp = ProjectOn_uvp(Nx, Ny, Nz, WTsgs)
                         
Buoy_res = np.zeros((Nx,Ny,Nz),'d',order='F')
Buoy_sgs = np.zeros((Nx,Ny,Nz),'d',order='F')

for k in range(1,Nz):
    Buoy_res[:,:,k] = (g_star/T0[k])*(WTuvp[:,:,k] - Wuvp[:,:,k]*T[:,:,k])
    Buoy_sgs[:,:,k] = (g_star/T0[k])*WTsgsuvp[:,:,k]
...
for k in range(1,Nz):
    Buoy_F[k] = (g_star/T0[k])*mean_xy(WTuvp-Wuvp*T)[k]
    Buoy_sgs_F[k] = (g_star/T0[k])*mean_xy(WTsgsuvp)[k]
...
Shear_res = - uu_var*dUdx - uv_var*dUdy - ProjectOn_uvp(Nx, Ny, Nz, uw_var*dUdz) \
            - uv_var*dVdx - vv_var*dVdy - ProjectOn_uvp(Nx, Ny, Nz, vw_var*dVdz) \
            - ProjectOn_uvp(Nx, Ny, Nz, uw_var*dWdx) - ProjectOn_uvp(Nx, Ny, Nz, vw_var*dWdy) - ProjectOn_uvp(Nx, Ny, Nz, ww_var)*dWdz
            
Shear_sgs = - txx*dUdx - tyy*dVdy - tzz*dWdz \
            - txy*(dUdy + dVdx) - ProjectOn_uvp(Nx, Ny, Nz, txz*(dUdz + dWdx)) - ProjectOn_uvp(Nx, Ny, Nz, tyz*(dVdz+dWdy))
            
Shear_tot = Shear_res + Shear_sgs

#Shear Prod Finnigan
Sh_F = - mean_xy(ProjectOn_uvp(Nx, Ny, Nz, uw_var))*ProjectOn_uvp_1D(Nz,mean_xy(dUdz)) \
       - mean_xy(ProjectOn_uvp(Nx, Ny, Nz, vw_var))*ProjectOn_uvp_1D(Nz,mean_xy(dVdz)) \
       - mean_xy(ProjectOn_uvp(Nx, Ny, Nz, ww_var))*mean_xy(dWdz)

Sh_sgs_F = - mean_xy(ProjectOn_uvp(Nx, Ny, Nz, txz))*ProjectOn_uvp_1D(Nz, mean_xy(dUdz)) \
           - mean_xy(ProjectOn_uvp(Nx, Ny, Nz, tyz))*ProjectOn_uvp_1D(Nz, mean_xy(dVdz)) \
           - mean_xy(tzz)*mean_xy(dWdz)
...
Spd = - uud*duddx - vvd*dvddy - ProjectOn_uvp(Nx, Ny, Nz, wwd)*dwddz - ProjectOn_uvp(Nx, Ny, Nz, uwd*(duddz+dwddx)) \
      - ProjectOn_uvp(Nx, Ny, Nz, vwd*(dvddz+dwddy)) - uvd*(duddy+dvddx)
Spd_xyavg = mean_xy(Spd)

# Dispersive Shear Production SGS
Spd_sgs = - txxd*duddx - tyyd*dvddy - tzzd*dwddz - ProjectOn_uvp(Nx, Ny, Nz, txzd*(duddz+dwddx)) \
          - ProjectOn_uvp(Nx, Ny, Nz, tyzd*(dvddz+dwddy)) - txyd*(duddy+dvddx)
...
uww_var = ProjectOn_uvp(Nx, Ny, Nz, UWW) - 2*ProjectOn_uvp(Nx, Ny, Nz, UW*W) - U*ProjectOn_uvp(Nx, Ny, Nz, WW) + 2*U*ProjectOn_uvp(Nx, Ny, Nz, W*W)
...
wuu_var = WUU - 2*UW*ProjectOn_w(Nx, Ny, Nz, U) - W*ProjectOn_w(Nx, Ny, Nz, UU) + 2*W*ProjectOn_w(Nx, Ny, Nz, U*U)
wvv_var = WVV - 2*VW*ProjectOn_w(Nx, Ny, Nz, V) - W*ProjectOn_w(Nx, Ny, Nz, VV) + 2*W*ProjectOn_w(Nx, Ny, Nz, V*V)
www_var = WWW - 3*WW*W + 2*W*W*W

ttx = - Fddx(Nx, Ny, Nz, 0.5*(uuu_var + uvv_var + uww_var), 1, 0, Lx, Nx)
tty = - Fddy(Nx, Ny, Nz, 0.5*(vuu_var + vvv_var + vww_var), 1, 0, Ly, Ny)
ttz = - ddz3_w(Nx, Ny, Nz, dz, 0.5*(wuu_var + wvv_var + www_var))
...
tt_tot_sgs = ttx_sgs + tty_sgs + ttz_sgs
...
# dttz_sgs = mean_xy()
Dtt_sgs_F = - 0.5*ddz_1D_w(Nz, dz, dttz_sgs)
...
Ttz_F = - ddz_1D_w(Nz, dz, mean_xy(0.5*(wuu_var + wvv_var + www_var)))
Ttz_sgs_F = - ddz_1D_w(Nz, dz, mean_xy(0.5*((Utxz - ProjectOn_w(Nx, Ny, Nz, U)*txz) + (Vtyz - ProjectOn_w(Nx, Ny, Nz, V)*tyz) + (Wtzz - W*ProjectOn_w(Nx, Ny, Nz, tzz)))))
...
ax.plot(Ttz_F+Dtt_F,z , color='green', ls=':', label=r'$TT_{res,F+D}$', marker='.')
...
var1 = pu3_var - 0.5*(UUU+UVV+ProjectOn_uvp(Nx, Ny, Nz, UWW)) - (1/3)*(Utxx+Utyy+Utzz) \
        + 0.5*(U*UU+U*VV+U*ProjectOn_uvp(Nx, Ny, Nz, WW)) \
        + (1/3)*(U*txx+U*tyy+U*tzz)
        
var2 = pv3_var - 0.5*(VUU+VVV+ProjectOn_uvp(Nx, Ny, Nz, VWW)) - (1/3)*(Vtxx+Vtyy+Vtzz) \
        + 0.5*(V*UU+V*VV+V*ProjectOn_uvp(Nx, Ny, Nz, WW)) \
        + (1/3)*(V*txx+V*tyy+V*tzz)

var3 = pw3_var - 0.5*(WUU+WVV+WWW) - (1/3)*(Wtxx+Wtyy+Wtzz) \
        + 0.5*(W*ProjectOn_w(Nx, Ny, Nz, UU)+W*ProjectOn_w(Nx, Ny, Nz, VV)+W*WW) \
        + (1/3)*(W*ProjectOn_w(Nx, Ny, Nz, txx)+W*ProjectOn_w(Nx, Ny, Nz, tyy)+W*ProjectOn_w(Nx, Ny, Nz, tzz))
        
dpu1dx = Fddx(Nx, Ny, Nz, var1, 1, 0, Lx, Nx)
dpv1dy = Fddy(Nx, Ny, Nz, var2, 1, 0, Ly, Ny)
dpw1dz = ddz3_w(Nx, Ny, Nz, dz, var3)
...
# dpu3dx = Fddx(Nx, Ny, Nz, pu3_var, 1, 0, Lx, Nx)
# dpv3dy = Fddy(Nx, Ny, Nz, pv3_var, 1, 0, Ly, Ny)
# dpw3dz = ddz3_w(Nx, Ny, Nz, dz, pw3_var)
...
Pt1_F = - ddz_1D_w(Nz, dz, mean_xy(pw1_var))
# Pt2_F = - ddz_1D_w(Nz, dz, mean_xy(pw2_var))

#Pressure Transport Profiles
figure, ax = plt.subplots(figsize=(10,8))

ax.plot(mean_xy(pres_trans_1),z, color='red', ls='--', label=r'$PT_{res}$',marker='.')
# ax.plot(mean_xy(dpv3dy),z, color='green', ls='--', label=r'$dpvdy_{res}$',marker='.')
# ax.plot(mean_xy(dpw3dz),z, color='black', ls='--', label=r'$dpwdz_{res}$',marker='.')
...
diss = dxx_tot + dyy_tot + dzz_tot + dxy_tot + ProjectOn_uvp(Nx, Ny, Nz, dxz_tot) + ProjectOn_uvp(Nx, Ny, Nz, dyz_tot)
...
ax.plot(mean_xy(diss),z, color='red', ls='--', label=r'$\epsilon$', marker='.')
...
# ax.plot(mean_xy(dxz_tot),z, color='blue', marker='.')
# ax.plot(mean_xy(dyz_tot),z, color='green', marker='.')
...
ax.plot(mean_xy(Buoy_tot),z, color='green', ls=':', label=r'$Buoy_{tot}$')
ax.plot(mean_xy(Shear_tot),z, color='orange', ls='-.', label=r'$SH_{tot}$')
ax.plot(mean_xy(tt_tot),z, color='grey', ls='-', label=r'$TT_{tot}$')
ax.plot(mean_xy(pres_trans_1),z, color='blue', ls='--', label=r'$PT_{res}$')
# ax.plot(mean_xy(diss+Shear_tot),z, color ='blue')
ax.plot(mean_xy(diss+Shear_tot+Buoy_tot+Adv_tot+tt_tot+pres_trans_1),z, color='green', ls='-', label=r'$Residual$')
...
#%%
#Work of the time-averaged velocity spatial fluctuations against the Double-Averaged shear stress
Pm_G = - ProjectOn_uvp_1D(Nz, mean_xy(uw_var)*mean_xy(duddz)) - ProjectOn_uvp_1D(Nz, mean_xy(vw_var)*mean_xy(dvddz)) - ProjectOn_uvp_1D(Nz, mean_xy(ww_var))*mean_xy(dwddz)

#SGS transport
# SGS_t_G = - ddz_1D_w(Nz, dz, mean_xy((Utxz - ProjectOn_w(Nx, Ny, Nz, U)*txz) \
#                                         + (Vtyz - ProjectOn_w(Nx, Ny, Nz, V)*tyz) + (Wtzz - W*ProjectOn_w(Nx, Ny, Nz, tzz))))

figure, ax = plt.subplots(figsize=(10,8))

ax.plot(Pm_G,z, color='red', ls='-', label=r'$Pm_{G}$')
# ax.plot(SGS_t_G,z, color='black', ls='-', label=r'$SGS_{t,G}$')

ax.set_xlabel(r'$\frac{\partial <e>}{\partial t}$', fontsize='15')
ax.set_ylabel(r'$z/z_i$', fontsize='15')
ax.set_title(r'Additional terms - Homog Surf T:300K', fontsize='15')
ax.axvline(0,color='black',linewidth=0.5)
ax.axhline(0,color='black',linewidth=0.5)
ax.legend(loc='upper right', fontsize='10')
