...
# -*- coding: utf-8 -*-
"""
Created on Wed Apr  2 11:15:29 2025
...
del data, dataNAN_flat, dataNAN_sin, dataNAN_atto, data_atto, data_sin, data_flat
...
d_dim_flat = compute_d_twr(dataNAN['flat'],coord['flat'],dist['flat'],height,dz,zi,u_scale,LAD,'ATTO')
d_dim_sin_min = compute_d_twr(dataNAN['sin_min'],coord['sin_min'],dist['sin_min'],height,dz,zi,u_scale,LAD,'ATTO')
d_dim_sin_max = compute_d_twr(dataNAN['sin_max'],coord['sin_max'],dist['sin_max'],height,dz,zi,u_scale,LAD,'ATTO')
d_dim_atto_min = compute_d_twr(dataNAN['atto_min'],coord['atto_min'],dist['atto_min'],height,dz,zi,u_scale,LAD,'ATTO')
d_dim_atto_max = compute_d_twr(dataNAN['atto_max'],coord['atto_max'],dist['atto_max'],height,dz,zi,u_scale,LAD,'ATTO')
...
for c in range(len(cases)):
    ...
    ustar2D = np.zeros((len(coord[cases[c]]),Nz_SLayer),'d',order='F')
    ustar = np.zeros((len(coord[cases[c]])),'d',order='F')
    z_over_d = np.zeros((len(coord[cases[c]]),Nz_SLayer),'d',order='F')
    
    Ruw = (dataNAN[cases[c]]['uw'] - dataNAN[cases[c]]['u']*dataNAN[cases[c]]['w'] - dataNAN[cases[c]]['txz'])*(u_scale**2)
    Rvw = (dataNAN[cases[c]]['vw'] - dataNAN[cases[c]]['v']*dataNAN[cases[c]]['w'] - dataNAN[cases[c]]['tyz'])*(u_scale**2)
    
    tmpDUDZ = ((dataNAN[cases[c]]['u']*dataNAN[cases[c]]['dudz'] + dataNAN[cases[c]]['v']*dataNAN[cases[c]]['dvdz'])/\
                np.sqrt(dataNAN[cases[c]]['u']**2 + dataNAN[cases[c]]['v']**2))*(u_scale/zi)
    ...
    for i in range(len(coord[cases[c]])):
        ...
        # ustar2D[i,:] = ((Ruw[loc[0],loc[1],int(np.where(dist[cases[c]][loc[0],loc[1],:]>0)[0][0]): \
        #                                       int(np.where(dist[cases[c]][loc[0],loc[1],:]>0)[0][0])+Nz_SLayer])**2 + \
        #                 (Rvw[loc[0],loc[1],int(np.where(dist[cases[c]][loc[0],loc[1],:]>0)[0][0]): \
        #                                      int(np.where(dist[cases[c]][loc[0],loc[1],:]>0)[0][0])+Nz_SLayer])**2)**(1/4)
            
        # phi_m_2D[i,:] = ((kappa*z_d[0:Nz_SLayer])/ustar[i,:])*tmpDUDZ[loc[0],loc[1],int(np.where(dist[cases[c]][loc[0],loc[1],:]>0)[0][0]): \
        #                      int(np.where(dist[cases[c]][loc[0],loc[1],:]>0)[0][0])+Nz_SLayer]
    ...
    for cluster in range(1,Nclusters):
        ...
        if (np.count_nonzero(np.isnan(yB_median[cluster,0:-1])) < Nz_SLayer-1):
            sc = axs[c].scatter(phiM_median[cluster,0:-1],(y_ax[0:Nz_SLayer-1]),marker='o',c = yB_median[cluster,0:-1],cmap=cmap,vmin=0,vmax=np.sqrt(3)/2)
    
    axs[c].plot(np.nanmean(phi_m_2D[:,:],axis=(0)),(y_ax[0:Nz_SLayer]),linestyle='-',color='k')
    
    if c==0:
        axs[c].set_position([0.06, 0.32, 0.2, 0.38])
    else:
        axs[c].set_position(positions[c-1])
    
    axs[c].set_xlim(-0.5, 2.5)
    axs[c].set_ylim(y_ax[0], 20)
    axs[c].axhline(y = (canopyH/canopyH), xmin=-10, xmax=8,color='gray',linestyle=':')
    # axs.axhline(y = 3*(canopyH/canopyH), xmin=-10, xmax=8,color='gray',linestyle='-.')
    
    axs[c].set_xscale('linear')
    # cbar = plt.colorbar(sc)
    axs[c].set_ylabel(r'$z/h_C$', fontsize=15)
    axs[c].set_xlabel(r'$\phi_M$',fontsize=15)
    axs[c].tick_params(axis='both', which='major', labelsize=12)
    axs[c].set_title(f'{cases[c]}',fontsize=15)
    
    axs[c].grid(which='major', axis='both',color='k', alpha = 0.3, linestyle='-')
    axs[c].grid(which='minor', axis='both',color='grey', alpha = 0.2, linestyle='-')
