...
# -*- coding: utf-8 -*-
"""
Created on Wed May 28 08:11:47 2025
...
Nfiles = 52
psiM_a = np.zeros((Nfiles),order='F')
psiM_na = np.zeros((Nfiles),order='F')

psiH_a = np.zeros((Nfiles),order='F')
psiH_na = np.zeros((Nfiles),order='F')

for i in range(0,Nfiles):
    psiM = xr.open_dataarray(path_a+'Data_Scalar_2D_'+str(StartTime + AvgIter*i)+'.nc').data[:,:,3] #momentum
    psiH = xr.open_dataarray(path_a+'Data_Scalar_2D_'+str(StartTime + AvgIter*i)+'.nc').data[:,:,5] #heat
    
    psiM_a[i] = np.mean(psiM)
    psiH_a[i] = np.mean(psiH)
    
    psiM = xr.open_dataarray(path_na+'Data_Scalar_2D_'+str(StartTime + AvgIter*i)+'.nc').data[:,:,3]
    psiH = xr.open_dataarray(path_na+'Data_Scalar_2D_'+str(StartTime + AvgIter*i)+'.nc').data[:,:,5]
    
    psiM_na[i] = np.mean(psiM)
    psiH_na[i] = np.mean(psiH)
...
p1 = axs[0].plot(mdates.date2num(time_vector),psiM_a,c='k',label='Aniso')
p1 = axs[0].plot(mdates.date2num(time_vector),psiM_na,c='r',label='NoAniso')
p2 = axs[1].plot(mdates.date2num(time_vector),psiH_a,c='k')
p2 = axs[1].plot(mdates.date2num(time_vector),psiH_na,c='r')
    
axs[0].set_ylabel(r"$\psi_M$",fontsize=15)
axs[1].set_ylabel(r"$\psi_H$",fontsize=15)
axs[1].set_xlabel('Time',fontsize=15)
for i in range(len(axs)):
    axs[i].set_ylim(-2,0.75)
...
# plt.savefig(path_fig+'psi_ts.png',dpi=300)
plt.show()