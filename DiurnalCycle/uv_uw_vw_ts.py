...
# -*- coding: utf-8 -*-
"""
Created on Wed May 28 06:44:44 2025
...
sigma_a = np.zeros((nz,Nfiles),order='F')
sigma_na = np.zeros((nz,Nfiles),order='F')
...
for i in range(0,Nfiles):
    ...
    # uv = xr.open_dataarray(path_a+'Data_Momentum_'+str(StartTime + AvgIter*i)+'.nc').data[:,:,:,13]
    # uw = xr.open_dataarray(path_a+'Data_Momentum_'+str(StartTime + AvgIter*i)+'.nc').data[:,:,:,14]
    vw = xr.open_dataarray(path_a+'Data_Momentum_'+str(StartTime + AvgIter*i)+'.nc').data[:,:,:,15]
    
    # txy = xr.open_dataarray(path_a+'Data_Momentum_'+str(StartTime + AvgIter*i)+'.nc').data[:,:,:,19]
    # txz = xr.open_dataarray(path_a+'Data_Momentum_'+str(StartTime + AvgIter*i)+'.nc').data[:,:,:,20]
    tyz = xr.open_dataarray(path_a+'Data_Momentum_'+str(StartTime + AvgIter*i)+'.nc').data[:,:,:,21]
    
    # sigma_a[:,i] = np.mean(uv - u*v - txy,axis=(0,1))
    # sigma_a[:,i] = np.mean(uw - u_w*w - txz,axis=(0,1))
    sigma_a[:,i] = np.mean(vw - v_w*w - tyz,axis=(0,1))
    ...
    # uv = xr.open_dataarray(path_na+'Data_Momentum_'+str(StartTime + AvgIter*i)+'.nc').data[:,:,:,13]
    # uw = xr.open_dataarray(path_na+'Data_Momentum_'+str(StartTime + AvgIter*i)+'.nc').data[:,:,:,14]
    vw = xr.open_dataarray(path_na+'Data_Momentum_'+str(StartTime + AvgIter*i)+'.nc').data[:,:,:,15]
    
    # txy = xr.open_dataarray(path_na+'Data_Momentum_'+str(StartTime + AvgIter*i)+'.nc').data[:,:,:,19]
    # txz = xr.open_dataarray(path_na+'Data_Momentum_'+str(StartTime + AvgIter*i)+'.nc').data[:,:,:,20]
    tyz = xr.open_dataarray(path_na+'Data_Momentum_'+str(StartTime + AvgIter*i)+'.nc').data[:,:,:,21]
    
    # sigma_na[:,i] = np.mean(uv - u*v - txy,axis=(0,1))
    # sigma_na[:,i] = np.mean(uw - u_w*w - txz,axis=(0,1))
    sigma_na[:,i] = np.mean(vw - v_w*w - tyz,axis=(0,1))
...
p1 = axs[0].pcolormesh(mdates.date2num(time_vector),np.arange(0,nz)*dz*zi,sigma_a*(uscale**2)/(ug**2)*1000,vmin=-4,vmax=2,cmap='hot',shading='auto')
p2 = axs[1].pcolormesh(mdates.date2num(time_vector),np.arange(0,nz)*dz*zi,sigma_na*(uscale**2)/(ug**2)*1000,vmin=-4,vmax=2,cmap='hot',shading='auto')
...
cbar.set_label(label=r'$\sigma_{vw}^2/u_G^2$',fontsize=15)
cbar = plt.colorbar(p2)
cbar.set_label(label=r'$\sigma_{vw}^2/u_G^2$',fontsize=15)

# plt.savefig(path_fig+'sigmaVW_ts_prof.png',dpi=300)
plt.show()
...
for i in range(len(height)):
    p1 = axs[0].plot(mdates.date2num(time_vector),sigma_a[height[i],:]*(uscale**2)/(ug**2)*1000,label=f'{height[i]*dz*zi:.2f}')
    p2 = axs[1].plot(mdates.date2num(time_vector),sigma_na[height[i],:]*(uscale**2)/(ug**2)*1000,label=f'{height[i]*dz*zi:.2f}')
    
axs[0].set_ylabel(r"$\sigma_{vw}^2/u_G^2$",fontsize=15)
axs[1].set_ylabel(r"$\sigma_{vw}^2/u_G^2$",fontsize=15)
axs[1].set_xlabel('Time',fontsize=15)
...
# plt.savefig(path_fig+'sigmaVW_ts_heights.png',dpi=300)
plt.show()