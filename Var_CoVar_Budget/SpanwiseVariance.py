...
# -*- coding: utf-8 -*-
"""
Created on Wed May 22 13:39:26 2024
...
#ADVECTION
terms_bdg['adv_h'] = - data_tavg['u']*get_dphidx(terms_ptb['v2_t'], wn_x) - data_tavg['v']*get_dphidy(terms_ptb['v2_t'], wn_y)
terms_bdg['adv_v'] = - data_tavg['w']*wnode2uvpnode(get_dphidz(terms_ptb['v2_t'], dz))

terms_bdg['adv_h_SGS'] = - data_tavg['u']*get_dphidx(data_tavg['tyy'], wn_x) - data_tavg['v']*get_dphidy(data_tavg['tyy'], wn_y)
terms_bdg['adv_v_SGS'] = - data_tavg['w']*wnode2uvpnode(get_dphidz(data_tavg['tyy'], dz))

terms_bdg['adv'] = terms_bdg['adv_h'] + terms_bdg['adv_v'] + terms_bdg['adv_h_SGS'] + terms_bdg['adv_v_SGS']

#PRODUCTION
terms_bdg['prod_h'] = - 2*(terms_ptb['uv_t']*data_tavg['dvdx'] + terms_ptb['v2_t']*data_tavg['dvdy'])
terms_bdg['prod_v'] = - 2*(terms_ptb['vw_t']*data_tavg['dvdz'])

terms_bdg['prod_h_SGS'] = - 2*(data_tavg['txy']*data_tavg['dvdx'] + data_tavg['tyy']*data_tavg['dvdy'])
terms_bdg['prod_v_SGS'] = - 2*(data_tavg['tyz']*data_tavg['dvdz'])
...
terms_ptb['v2u_t'] = data_tavg['vvu'] - 2*data_tavg['v']*data_tavg['uv']\
  + 2*data_tavg['u']*(data_tavg['v']**2) - data_tavg['u']*data_tavg['vv']
terms_ptb['vtxy_t'] = data_tavg['vtxy'] - data_tavg['v']*data_tavg['txy']

terms_bdg['uturb_h'] = - get_dphidx(terms_ptb['v2u_t'], wn_x) - get_dphidy(terms_ptb['v3_t'], wn_y) - get_dphidx(terms_ptb['vtxy_t'], wn_x)\
    - get_dphidy(terms_ptb['vtyy_t'], wn_y)
...
terms_bdg['uturb_v'] = - wnode2uvpnode(get_dphidz(terms_ptb['wv2_t'], dz)) - wnode2uvpnode(get_dphidz(terms_ptb['vtyz_t'], dz))

terms_bdg['ttrans'] = terms_bdg['uturb_h'] + terms_bdg['uturb_v']

#PRESSURE TRANSPORT
terms_ptb['pv_t'] = data_tavg['pv'] - data_tavg['p']*data_tavg['v']
terms_bdg['pturb_t'] = - 2*(get_dphidy(terms_ptb['pv_t'], wn_y))
terms_ptb['pdvdy_t'] = data_tavg['pdvdy'] - data_tavg['p']*data_tavg['dvdy']
terms_bdg['pturb_s'] = 2*terms_ptb['pdvdy_t']
...
data_tavg['dissip'] = data_tavg['dyy'] + data_tavg['dxy'] + data_tavg['dyz']

terms_bdg['dissip'] = 2*(data_tavg['dissip']\
  - data_tavg['tyy']*terms_drv['S22'] - data_tavg['txy']*terms_drv['S12'] - data_tavg['tyz']*terms_drv['S23'])
    
#CANOPY
terms_bdg['canopy'] = 2*(data_tavg['vfdy']-data_tavg['v']*data_tavg['fdy'])
...
# np.save(pathOUT + 'VV_terms.npy', terms_bdg) 
...
plt.xlim(-40,30)
plt.title(r'Spanwise Variance')
plt.xlabel(r'$\left \langle duu/dt \right \rangle$')
# plt.xlabel(r'$\left \langle de/dt \right \rangle$')
plt.ylabel(r'$z/h_c$')
plt.legend(loc='upper right')

# plt.savefig(pathOUT +'Figures/VV_budget_prof_flat.png',dpi=300,facecolor='white', edgecolor='white')
