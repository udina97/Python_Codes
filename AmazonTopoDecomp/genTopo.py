...
# File
dt2_file = './data/s03_w061_1arc_v3.dt2'
...
loc_cen = (-2.1458, -59.0038) # midpoint between small and tall tower, used in Zev 11/29/21
...
npoint = 95 # generates LES domain of 100 grid points -- is 1 more than num input 
'''
npoint = 99 returns a domain of 3000x3000m because dx=30m
npoint = 95 returns a domain of 2880x2880m 
To know how many npoint you need do: Lx/30 - 1
'''
...
'''
xspan and yspan are the domain spans in units of km, 1/111 transforms 1 degree of latitude/longitude in km
'''
xr_lg = ((-60.0, -59.0), (loc_cen[-1]-0.12, loc_cen[-1]+0.12)) # [0] Large plot Long dimensions [1] dashed boundries
yr_lg = ((-2.50, -2.00), (loc_cen[0]-0.06, loc_cen[0]+0.06)) # [0] Large plot lon dimensions
...
if os.path.isfile(fn_hgt):
  if data_type == 'hgt':
    ...
    if not(combine_dt2):
      ...
      ele_amazon = np.hstack((ele_amazon1[:,:-1], ele_amazon2)) #combining the two topography files
      lat = lat1
...
ym, xm = np.mgrid[0:(npoint+1)*dx:dx, 0:(npoint+1)*dx:dx] #create meshgrid with LES domain size Lx and Ly
...
ele_org = ele_org-ele_org.min() #shift minimum to zero
...
mean = ele.mean()
ele = ele - ele.min() #ele is the same as ele_org but made periodic at the boundaries and where the minimum has been shifted to 0
...
levels_ele = np.arange(lbd_amazon[0], lbd_amazon[1]+1, 10)
...
levels = np.arange(lbd_les[0], lbd_les[1]+30, 5)
...
topodict = {'topo': ele}
savemat('./data/ATTO96.mat', topodict)