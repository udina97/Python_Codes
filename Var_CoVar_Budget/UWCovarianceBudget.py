...
# -*- coding: utf-8 -*-
"""
Created on Tue May 28 09:51:04 2024
...
pvar = ['pdudx','pdudy','pdudz','pdvdx','pdvdy','pdvdz','pdwdx','pdwdy','pdwdz','uvw','utyz','vtxz','wtxy']        
        
if load_pdata:
    with open(pathOUT+'data_covar.pkl', 'rb') as f:
    # Load the data from the pickle file
        data_p = pickle.load(f)
        
    data_p_tavg = dict()
    for i in range(len(pvar)):
        data_p_tavg[pvar[i]] = np.mean(data_p[pvar[i]],axis=3)
else:
    data_p = dict()
    data_p_tavg = dict()

    for i in range(len(pvar)):
        data_p[pvar[i]] = get_var(ipath,pvar[i],nx,ny,nzTot,nt_tot,iintf,mpiProc,avgT)
        data_p_tavg[pvar[i]] = np.mean(data_p[pvar[i]],axis=3)

if save_pdata:
    with open(pathOUT+'data_covar.pkl', 'wb') as f:
...
#%%Compute the Covariance budget terms

#ADVECTION
terms_bdg['adv_h'] = - data_tavg['u']*get_dphidx(terms_ptb['uw_t'], wn_x) - data_tavg['v']*get_dphidy(terms_ptb['uw_t'], wn_y)
terms_bdg['adv_v'] = - data_tavg['w']*wnode2uvpnode(get_dphidz(terms_ptb['uw_t'], dz))

terms_bdg['adv_h_SGS'] = - data_tavg['u']*get_dphidx(data_tavg['txz'], wn_x) - data_tavg['v']*get_dphidy(data_tavg['txz'], wn_y)
terms_bdg['adv_v_SGS'] = - data_tavg['w']*wnode2uvpnode(get_dphidz(data_tavg['txz'], dz))

terms_bdg['adv'] = terms_bdg['adv_h'] + terms_bdg['adv_v'] + terms_bdg['adv_h_SGS'] + terms_bdg['adv_v_SGS']

#PRODUCTION
terms_bdg['prod_h'] = - (terms_ptb['u2_t']*data_tavg['dwdx'] + terms_ptb['uv_t']*data_tavg['dwdy'] + terms_ptb['uw_t']*data_tavg['dudx']\
                         + terms_ptb['vw_t']*data_tavg['dudy'])
terms_bdg['prod_v'] = - (terms_ptb['uw_t']*data_tavg['dwdz'] + terms_ptb['w2_t']*data_tavg['dudz'])

terms_bdg['prod_h_SGS'] = - (data_tavg['txx']*data_tavg['dwdx'] + data_tavg['txy']*data_tavg['dwdy'] + data_tavg['txz']*data_tavg['dudx']\
                         + data_tavg['tyz']*data_tavg['dudy'])
terms_bdg['prod_v_SGS'] = - (data_tavg['txz']*data_tavg['dwdz'] + data_tavg['tzz']*data_tavg['dudz'])
...
terms_ptb['wu2_t'] = data_tavg['uuw'] - 2*u_h*data_tavg['uw'] + 2*data_tavg['w']*u_h**2\
  - data_tavg['w']*u2_h
terms_ptb['uvw_t'] = data_p_tavg['uvw'] - terms_ptb['uv_t']*data_tavg['w'] - terms_ptb['uw_t']*data_tavg['v'] - data_tavg['u']*terms_ptb['vw_t']\
                    + 2*data_tavg['u']*data_tavg['v']*data_tavg['w']
terms_ptb['utxz_t'] = data_tavg['utxz'] - data_tavg['u']*data_tavg['txz']
terms_ptb['utyz_t'] = data_p_tavg['utyz'] - data_tavg['u']*data_tavg['tyz']                    

terms_bdg['uturb_h'] = - get_dphidx(terms_ptb['wu2_t'], wn_x) - get_dphidx(terms_ptb['utxz_t'], wn_x) - get_dphidy(terms_ptb['uvw_t'], wn_y)\
                    - get_dphidy(terms_ptb['utyz_t'], wn_y)
...
terms_bdg['uturb_v'] = -get_dphidz(terms_ptb['uw2_t'], dz) - get_dphidz(terms_ptb['wtxz_t'], dz)      
...
terms_bdg['pturb_t'] = - (get_dphidx(terms_ptb['pw_t'], wn_x)) - get_dphidz(terms_ptb['pu_t'], dz)
terms_ptb['pdudz_t'] = data_p_tavg['pdudz'] - data_tavg['p']*data_tavg['dudz']
terms_ptb['pdwdx_t'] = data_p_tavg['pdwdx'] - data_tavg['p']*data_tavg['dwdx']
terms_bdg['pturb_s'] = terms_ptb['pdudz_t'] + terms_ptb['pdwdx_t']
...
var_bdg = ['adv_h', 'adv_v', 'adv_h_SGS', 'adv_v_SGS', 'uturb_h', 'uturb_v', 'pturb_t', 'pturb_s', 'prod_h', 'prod_v','adv','ptrans','ttrans','prod']
...
terms = ['adv','ptrans','ttrans','prod','pturb_s','prod_h','prod_v','pturb_t','uturb_h','uturb_v']
