...
from functions import load_3d_UCLAdata, load_UCLAnpy_files, read_out_files, load_2d_UCLAdata
...
data = load_UCLAnpy_files(path) #xr.open_dataarray(path+'Data_Momentum_4TKE.nc')

# NumVar_LES = 64
# NumVar = 49
uscale = 0.4 #For the 9 m/s case
h_canopy = 39
...
# data['p_r'] = data['p'] - 1/2*(data['u2'] + data['v2'] + wnode2uvpnode(data['w2'])) 
# data['up_r'] = data['pu'] - 1/2*(data['u3'] + data['uv2'] + wnode2uvpnode(data['uw2']))#\
#     #- 1/3*(data['utxx'] + data['utyy'] + data['utzz']) 
# data['vp_r'] = data['pv'] - 1/2*(data['u2v'] + data['v3'] + wnode2uvpnode(data['vw2']))#\
#     #- 1/3*(data['vtxx'] + data['vtyy'] + data['vtzz'])
# data['wp_r'] = data['pw'] - 1/2*(uvpnode2wnode(data['u2w']) + uvpnode2wnode(data['v2w']) + data['w3'])#\
...
np.save(pathOUT + 'TKE_terms.npy', terms_bdg) 
...
var_bdg = ['adv_h', 'adv_v', 'uturb_h', 'uturb_v', 'pturb_h', 'pturb_v', 'dissip', 'canopy', 
           'prod_h', 'prod_v']

for i in range(len(var_bdg)):
    terms_bdg[var_bdg[i]][(dist < 0)] = float("nan")
    terms_bdg_wnode[var_bdg[i]][(dist < 0)] = float("nan")
...
plt.figure()
plt.plot(np.nanmean(terms_bdg['dissip'],axis=(0,1)),z_ax,label='Dissip')
plt.plot(np.nanmean(terms_bdg['adv_h'],axis=(0,1)),z_ax,label='Advec_h')
plt.plot(np.nanmean(terms_bdg['adv_v'],axis=(0,1)),z_ax,label='Advec_v')
plt.plot(np.nanmean(terms_bdg['prod_h'],axis=(0,1)),z_ax,label='Prod_h')
plt.plot(np.nanmean(terms_bdg['prod_v'],axis=(0,1)),z_ax,label='Prod_v')
plt.plot(np.nanmean(terms_bdg['uturb_h'],axis=(0,1)),z_ax,label='T-Trans_h')
plt.plot(np.nanmean(terms_bdg['uturb_v'],axis=(0,1)),z_ax,label='T-Trans_v')
plt.plot(np.nanmean(terms_bdg['pturb_h'],axis=(0,1)),z_ax,label='p-Trans_h')
plt.plot(np.nanmean(terms_bdg['pturb_v'],axis=(0,1)),z_ax,label='p-Trans_v')
plt.plot(np.nanmean(terms_bdg['canopy'],axis=(0,1)),z_ax,label='Canopy')
plt.plot(np.nanmean(Budget,axis=(0,1)),z_ax,color='k',label='Budget')
...
plt.figure()
plt.plot(np.nanmean(terms_bdg_wnode['dissip'],axis=(0,1)),z_ax_wnode,label='Dissip')
plt.plot(np.nanmean(terms_bdg_wnode['adv_h'],axis=(0,1)),z_ax_wnode,label='Advec_h')
plt.plot(np.nanmean(terms_bdg_wnode['adv_v'],axis=(0,1)),z_ax_wnode,label='Advec_v')
plt.plot(np.nanmean(terms_bdg_wnode['prod_h'],axis=(0,1)),z_ax_wnode,label='Prod_h')
plt.plot(np.nanmean(terms_bdg_wnode['prod_v'],axis=(0,1)),z_ax_wnode,label='Prod_v')
plt.plot(np.nanmean(terms_bdg_wnode['uturb_h'],axis=(0,1)),z_ax_wnode,label='T-Trans_h')
plt.plot(np.nanmean(terms_bdg_wnode['uturb_v'],axis=(0,1)),z_ax_wnode,label='T-Trans_v')
plt.plot(np.nanmean(terms_bdg_wnode['pturb_h'],axis=(0,1)),z_ax_wnode,label='p-Trans_h')
plt.plot(np.nanmean(terms_bdg_wnode['pturb_v'],axis=(0,1)),z_ax_wnode,label='p-Trans_v')
plt.plot(np.nanmean(terms_bdg_wnode['canopy'],axis=(0,1)),z_ax_wnode,label='Canopy')
plt.plot(np.nanmean(Budget_wnode,axis=(0,1)),z_ax_wnode,color='k',label='Budget')
# plt.hlines(1,-1500,1500,linestyle='--',colors='k')
plt.ylim(0,10)
# plt.xlim(-1500,1500)
#plt.xlim(-40,80)
