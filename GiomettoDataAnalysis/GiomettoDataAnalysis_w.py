...
# -*- coding: utf-8 -*-
"""
Created on Fri Aug 29 14:33:21 2025
...
ustar = ((R13 + data['txz_tw'].squeeze())**2 + (R23 + data['tyz_tw'].squeeze())**2)**(0.25)
...
shear = np.sqrt((R13 + data['txz_tw'].squeeze())**2 + ((R23 + data['tyz_tw'].squeeze()))**2)/np.max(ustar)**2
...
phi = phi_m_loc(nz, z_d, u, v, data['dudz_tw'].squeeze(), data['dvdz_tw'].squeeze(), ustar_v2)
...
diss = data['ds_tw'].squeeze()*tke_nd
mtr = data['mtr_tw'].squeeze()*tke_nd
ptrans = data['pt_tw'].squeeze()*tke_nd
prod = data['sp_tw'].squeeze()*tke_nd
ttrans = data['tt_tw'].squeeze()*tke_nd
pmd = diss[8:] + prod[8:]
...
xB,yB,lambda3 = Anisotropy1D(R11, R22, R33, R12, R13, R23)
import xarray as xr 
aniso = xr.DataArray(np.ones(shape = (nz,2),order='F'),\
                       dims=('z','variable'), coords = {'variable':['xB','yB']})
aniso[:,0] = xB
aniso[:,1] = yB
# aniso.to_netcdf('simulation_G/anisotropy_w.nc')
