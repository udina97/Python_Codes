...
# -*- coding: utf-8 -*-
"""
Created on Mon Dec 16 04:07:25 2024
...
for j in range(len(cases)):
    ...
    for i in range(0,3):
        ...
        for k in range(len(coord[cases[j]])):
            loc = coord[cases[j]][k]
            ustar = np.sqrt((Ruw[loc[0],loc[1],int(np.where(dist[cases[j]][loc[0],loc[1],:]>0)[0][0])+16])**2 + \
                            (Rvw[loc[0],loc[1],int(np.where(dist[cases[j]][loc[0],loc[1],:]>0)[0][0])+16])**2)
            tmp[k,:] = velocity[cases[j]][var[i]][loc[0],loc[1],int(np.where(dist[cases[j]][loc[0],loc[1],:]>0)[0][0]): \
                                                  int(np.where(dist[cases[j]][loc[0],loc[1],:]>0)[0][0])+Nz_SLayer]*(u_scale/ustar)
    
        axs.plot(np.nanmean(tmp,axis=(0)),np.arange(0,Nz_SLayer)*dz/(39/zi) + ((dz/2)/(39/zi)),label=var[i],linewidth=3)
    
    axs.set_xlabel(r'$U_i/u_*$',fontsize=15)
    ...
    # plt.savefig(pathFig+f'{cases[j]}_VELprofTWR_ustar.png',dpi=300,facecolor='None', edgecolor='None')
