...
"""
Program: paperraw_crossProdDissipHill
The crossection of Production and dissipation terms for the hill case
"""

### Histories:
### 03/18/2021 -- Bicheng Chen (chabby@ucla.edu) -- First created
...
# Case
case = ('amazon_canopy_hill', 'amazon_canopy_nohill')
lab_case = ('hill', 'flat')
nm_case = ('idealized', 'flag')

# File format
fmt_lespath = '/data/2/bzc/LES/amazon_2D/{case:s}'
fn_budget = './figure/paperraw/paperraw_crossProdDissipHill.png'
...
labs_bdg = (r'$P$', r'$\epsilon$', r'$\epsilon_c$', r'$P-\epsilon-\epsilon_c$',
  r'$-A_e$', r'$-T_e$', r'$-\Pi_e$', 'sum')
ind_plot = [0, 1, 2, 3]
...
levels_bdg = np.arange(-1, 4+0.1, 0.2)
...
for ic in range(nca):
  for ind in ind_plot:
    terms_bdg[ic][items_bdg[ind]] *= hc/dm[ic].lz
    print(terms_bdg[ic][items_bdg[ind]].max(), terms_bdg[ic][items_bdg[ind]].min())
...
cmap_bdg = truncate_colormap(cm.get_cmap("seismic"), 0.4, 1, 60)
...
gs = gridspec.GridSpec(2, (nplot+1)//2, width_ratios=[9.5, 0.5, 9.5, 0.5])
gs.update(**sz)
...
for ic in range(nca):
  ...
  fn_npz = "./data/forpaper/fig2_crossProdDissip_{case:s}".format(case=nm_case[ic])
  items = tuple([items_bdg[ind] for ind in ind_plot])
  terms = dict.fromkeys(items, None)
  for item in items:
    terms[item] = terms_bdg[ic][item]
  np.savez(fn_npz, x=x_gd[ic], z=z_gd[ic], terms=terms)
...
cbar.ax.set_ylabel(r'$\frac{\partial \overline{e}}{\partial t} \frac{h_c}{u_*^3}$',
